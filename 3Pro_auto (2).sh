...
if [[ $IP =~ ^10\.0\.0\..* ]]; then
    ...
    # NIC=$(ip link show | awk -F': ' '/^[0-9]+/ {print $2}' | tail -n +2)
...
if [ -n "$NIC" ] && [ ! -f "$SCRIPT_PATH" ]; then
        echo "#!/bin/bash" > "$SCRIPT_PATH"
        echo "ifconfig $NIC:1 10.0.0.5 up" >> "$SCRIPT_PATH"
        echo "ifconfig $NIC:2 10.0.0.6 up" >> "$SCRIPT_PATH"
        echo "ifconfig $NIC:3 10.0.0.7 up" >> "$SCRIPT_PATH"
        echo "ifconfig $NIC:4 10.0.0.8 up" >> "$SCRIPT_PATH"
        echo "ifconfig $NIC:5 10.0.0.11 up" >> "$SCRIPT_PATH"
        echo "ifconfig $NIC:6 10.0.0.12 up" >> "$SCRIPT_PATH"
        echo "ifconfig $NIC:7 10.0.0.13 up" >> "$SCRIPT_PATH"
        echo "ifconfig $NIC:8 10.0.0.14 up" >> "$SCRIPT_PATH"
        echo "sudo /etc/init.d/3proxy start" >> "$SCRIPT_PATH"
        echo "exit" >> "$SCRIPT_PATH"
        chmod +x "$SCRIPT_PATH"
    fi
...
if ! grep -q -F -x "@reboot sleep 35 && bash $SCRIPT_PATH" /var/spool/cron/root; then
    echo "@reboot sleep 35 && bash $SCRIPT_PATH" >> /var/spool/cron/root
    fi
else
