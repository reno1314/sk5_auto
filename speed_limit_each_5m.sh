...
# 配置文件路径
config_file="/root/traffic_control_config.conf"
...
create_traffic_control() {
  ...
  echo "ip_addresses=(${ip_addresses[@]})" >> "$config_file"
  echo "default_limit=$default_limit" >> "$config_file"
}

# 删除限速规则
delete_traffic_control() {
  if [ ! -f "$config_file" ]; then
    echo "未找到配置文件，无法删除限速规则。"
    exit 1
  fi
